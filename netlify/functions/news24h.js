...
function timeAgo(ts){ const d=Date.now()-ts; const h=Math.floor(d/36e5); if(h<1) return 'just now'; if(h<24) return h+'h ago'; return Math.floor(h/24)+'d ago'; }
...
exports.handler = async () => {
  try{
    ...
    for(const url of feeds){
      try{
        ...
        (f.items||[]).forEach(it=>{
          ...
          items.push({ title: it.title||'', url: it.link||it.guid||'#', source: (f.title||'').replace(/\s+RSS.*$/i,''), ts:t, timeago: timeAgo(t), snippet: it.contentSnippet||it.content||'' });
        });
      }catch(e){}
    }
    items.sort((a,b)=> b.ts - a.ts);
    let top5 = items.slice(0,5).map(x => ({ title:x.title, url:x.url, source:x.source, explainer: explainerFrom(x.title, x.snippet) }));
    if(!items.length){
      const fallbackItems=[
    ...
    return { statusCode:200, headers:{'Cache-Control':'public, max-age=300'}, body: JSON.stringify({ items, top5 }) };
  }catch(e){
