...
function TradingView() {
  // Safe placeholder (we can re-enable the TV script later)
  return (
    <div className="card">
      <h2>Price Chart</h2>
      <p className="muted">
        TradingView temporarily disabled for reliability. We can re-enable once the site is stable.
      </p>
    </div>
  );
}
...
export default function Dashboard() {
  ...
  if (loading) {
    return (
      <div className="card">
        <h2>Loading…</h2>
        <p className="muted">Fetching BTC price, mempool, fees and difficulty.</p>
      </div>
    );
  }
  if (!data) {
    return (
      <div className="card">
        <h2>Error</h2>
        <p className="muted">Could not load dashboard data.</p>
      </div>
    );
  }
